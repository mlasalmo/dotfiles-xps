...
# ARG_HELP([Tests the installation of the dotfiles in different scenarios],[])
# ARGBASH_SET_INDENT([  ])
# ARGBASH_GO()
...
print_help() {
  printf "%s\n" "Tests the installation of the dotfiles in different scenarios"
...
EOF
    ...
    gnome)
      run_test "${os}" "$(
        ...
        cat <<'EOF'
sudo apt-get update
sudo DEBIAN_FRONTEND=noninteractive apt-get install --yes --no-install-recommends gnome-shell
EOF
      )"
      ;;
