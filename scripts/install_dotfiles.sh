...
DOTFILES_USER=${DOTFILES_USER:-"mlasalmo"}
DOTFILES_REPO="${DOTFILES_REPO_HOST}/${DOTFILES_USER}/dotfiles-xps"
...
if ! command -v git > /dev/null 2>&1; then
  ...
  sudo apt-get update
  sudo DEBIAN_FRONTEND=noninteractive apt-get install --yes --no-install-recommends git
fi
